...
export async function POST(request: NextRequest) {
  ...
  if (profile.credits < 2) return NextResponse.json({ error: 'Insufficient credits. Please upgrade your plan.' }, { status: 402 });
  ...
  try {
    ...
    const input: any = { prompt, image: imageUrl, negative_prompt: negativePrompt || '' };
    ...
    const output = await replicate.run('stability-ai/sdxl-inpainting' as any, { input }) as any;
